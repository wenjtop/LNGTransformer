...
from tool.datasets import build_dataset
from tool.engine import train_one_epoch, evaluate
...
def get_args_parser():
    ...
    parser.add_argument('--min-lr', type=float, default=5e-6, metavar='LR',
                        help='lower lr bound for cyclic schedulers that hit 0 (1e-5)')  #lsg：1e-5   # swin 5e-6
    ...
    parser.add_argument('--color-jitter', type=float, default=0.4, metavar='PCT',        # 色彩抖动
    ...
    # Dataset parameters
    parser.add_argument('--data-path', default='../imagenet-100', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET100', choices=['CIFAR', 'IMNET', 'IMNET50', 'IMNET100', 'INAT', 'INAT19'],
...
def main(args):
    ...
    model = getattr(create_model, args.model)(num_classes=args.nb_classes, drop_path_rate=args.drop_path)
    ...
    print('params:', params)
    print('flops:', flops)
    pf = 'params:'+str(params)+'\n'+'flops:'+str(flops)+'\n'
    output_dir = Path(args.output_dir)
    logname = args.model+'_'+time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())+'.txt'
    if args.output_dir and utils.is_main_process():
        with (output_dir / logname).open("a") as f:
            f.write(str(args) + "\n" + pf)
    ...
    linear_scaled_warmup_lr = args.warmup_lr * 256 * utils.get_world_size() / 512.0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        remaining_time = (args.epochs - epoch) * total_time
